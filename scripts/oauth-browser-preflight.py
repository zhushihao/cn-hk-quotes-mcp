"""Credential-free browser mechanism test; NOT real ChatGPT/Automation acceptance.

Three ephemeral loopback HTTP servers model issuer, allowed callback and denied
callback origins. No production URLs or credentials are requested. Actual server
receipts (not Playwright route mocks) prove whether the browser followed a 302.
"""
import json
import re
import threading
...
if not MATCH:
    raise RuntimeError("Authorization-page CSP not found: update test with implementation")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def receipt(self, status, body_present=False):
        with LOCK:
            STATE["events"].append({"role": self.server.role, "method": self.command,
                                    "path": urlsplit(self.path).path,
                                    "status": status, "body_present": body_present})

    def do_GET(self):
        if self.server.role == "issuer" and self.path == "/authorize":
            self.receipt(200)
            self.respond(200, b'<html><form method="POST" action="/authorize">'
...
def run_case(browser, name, policy, issuer, callback, expected, enter=False, with_iss=False):
    ...
    try:
        ...
        for _ in range(20):
            ...
            with LOCK:
                done = any(e["role"] != "issuer" for e in STATE["events"])
    ...
    result = {"case": name, "callback_received": actual, "expected_callback": expected,
              "server_returned_302": posted, "csp_form_action_blocked": csp_blocked,
              "callback_has_form_body": leaked_form, "driver_error": failure,
              "server_receipts": events, "result": "PASS" if passed else "FAIL"}
    context.close()
    return result
...
try:
    ...
    with sync_playwright() as playwright:
        ...
        cases = [
            ...
            run_case(browser, "candidate_unapproved_origin_denied", candidate, issuer,
                     denied + "/callback", False),
        ]
        report = {"kind": "SYNTHETIC_BROWSER_MECHANISM_TEST", "browser": browser.version,
                  "network": "loopback HTTP only; separate issuer/callback origins",
                  "current_source_allows_chatgpt": "https://chatgpt.com" in CURRENT,
                  "real_chatgpt_acceptance": "NOT_RUN", "automation_acceptance": "NOT_RUN",
                  "cases": cases}
        browser.close()
