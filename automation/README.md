...
3. Collector 只提供业务数据、LIVE facts、Research replica 与 Research Job 协议；不得承载或分发 Automation Prompt。
4. Web 只用于业务 Prompt 明确要求的最新外部事实扫描；不得承担 Prompt/Guidance 控制面读取。
5. GitHub connector 仅用于业务账本等明确授权的运行时读写，例如 holding-assistant 的 Issue #2 append-only 状态账本。
